...
const GOOGLE_FALLBACK_MODELS = [
  ...
  "gemini-2.5-flash",
  "gemini-2.5-pro"
]; // Google generateContent-compatible ids. [web:293]

const OR_FALLBACK_MODELS = [
  "google/gemini-2.5-pro",
  "google/gemini-2.0-flash-001"
]; // OpenRouter catalog examples. [web:342][web:344]

const COMET_FALLBACK_MODELS = [
  "google/gemini-2.5-flash-image",
  "google/gemini-2.5-pro",
  "google/gemini-2.0-flash-001"
]; // CometAPI catalog examples. [web:390][web:393]
...
document.getElementById('export').onclick = () => {
  ...
  PP.postMessage('app.activeDocument.saveToOE("png");', '*'); // Export PNG buffer. [web:40]
};
...
refreshBtn.onclick = async () => {
  ...
  if (provider === 'google')      await loadModelsGoogle(key);
  else if (provider === 'openrouter') await loadModelsOpenRouter();
  else                             await loadModelsComet();
};
...
document.getElementById('generate').onclick = async () => {
  try {
    ...
    for (let i = 0; i < count; i++) {
      let outBuf;
      if (provider === 'google')       outBuf = await genGoogleWithBackoff(apiKey, chosen, promptEl.value || 'Enhance image', b64);
      else if (provider === 'openrouter') outBuf = await genOpenRouter(apiKey, chosen, promptEl.value || 'Enhance image', b64);
      else                               outBuf = await genComet(apiKey, chosen, promptEl.value || 'Enhance image', b64);

      if (outBuf) {
        PP.postMessage(outBuf, '*'); // Insert result. [web:40]
        setStatus(`Inserted image ${i + 1}/${count}`);
      }
      await delay(6500); // polite pacing to reduce 429s. [web:249]
    }
...
async function loadModelsGoogle(apiKey) {
  ...
  try {
    ...
    list = arr
      ...
      .filter(Boolean); // Keep generateContent-capable. [web:282][web:293]
  } catch (e) {
...
async function loadModelsOpenRouter() {
  ...
  modelSelect.innerHTML = '';
  fillModelSelect([...OR_FALLBACK_MODELS]); // Catalog is on site; use curated list + custom box. [web:342][web:344]
}

async function loadModelsComet() {
  setStatus('Listing CometAPI models...');
  modelSelect.innerHTML = '';
  fillModelSelect([...COMET_FALLBACK_MODELS]); // Use curated list + custom box. [web:390][web:393]
}
...
// ----- Google generate (inline image) -----

async function genGoogleWithBackoff(apiKey, modelId, prompt, inputB64) {
  let attempt = 0;
  while (true) {
    try {
      return await genGoogle(apiKey, modelId, prompt, inputB64);
    } catch (e) {
      const msg = String(e && e.message || '');
      if (msg.includes('HTTP 429')) {
        const delaySec = parseRetryDelaySeconds(msg) || 40;
        setStatus(`429 from ${modelId}, waiting ${delaySec}s...`);
        await delay(delaySec * 1000); // RetryInfo. [web:260]
        attempt++; continue;
      }
      throw e;
    }
...
async function genGoogle(apiKey, modelId, prompt, inputB64) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(modelId)}:generateContent`; // [web:293]
  const body = {
    contents: [{ parts: [
      { text: prompt },
      { inline_data: { mime_type: 'image/png', data: inputB64 } }
    ]}]
  }; // Image edit with inline_data. [web:396]
...
// ----- OpenRouter generate (messages schema) -----

async function genOpenRouter(apiKey, modelId, prompt, inputB64) {
  const url = 'https://openrouter.ai/api/v1/chat/completions'; // Messages schema is widely supported. [web:344]
  const body = {
    model: modelId,
    messages: [
      {
        ...
        content: [
          ...
          { type: 'image_url', image_url: { url: `data:image/png;base64,${inputB64}`, detail: 'auto' } } // detail per docs. [web:418]
        ]
      }
    ]
  };
  ...
  const json = JSON.parse(txt);
  // Try to extract an image; many OR models return text only.
  const out = json?.choices?.[0]?.message?.content;
  const b64 = findBase64Image(out);
  if (!b64) throw new Error('No image returned (try a different OpenRouter model id).');
  return base64ToArrayBuffer(b64);
}

// ----- CometAPI generate (OpenAI‑compatible responses) -----

async function genComet(apiKey, modelId, prompt, inputB64) {
  const url = 'https://api.cometapi.com/v1/responses'; // CometAPI unified endpoint. [web:393]
  const body = {
    model: modelId,
    input: [{
      role: 'user',
  ...
  const b64 = findBase64Image(out);
  if (!b64) throw new Error('No image returned (try another Comet model id).');
  return base64ToArrayBuffer(b64);
}
...
function parseRetryDelaySeconds(msg) {
  ...
  return m ? parseInt(m[1], 10) : null; // Google RetryInfo parsing. [web:260]
}
